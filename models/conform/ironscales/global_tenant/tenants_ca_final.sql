...
with tenants_ae as (
    select * from {{ ref('tenants_ca_tree')}}
),

campaigns_companylicense as (
    select * from {{ ref('stg_ironscales_ca_campaigns_companylicense_table')}}
    where
...
campaigns_company as (
    select * from {{ ref('stg_ironscales_ca_campaigns_company_table')}}
    where
    date(_RIVERY_LAST_UPDATE) = current_date()
...
auth_user as (
    select * from {{ ref('stg_ironscales_ca_auth_user_table')}}
    where
    date(_RIVERY_LAST_UPDATE) = current_date()
),

active_profiles as (
    select * from {{ ref('tenants_ca_active_profile')}}
),

shared_profiles as (
    select * from {{ ref('tenants_ca_shared_profile')}}
),

setting as (
    select * from {{ ref('stg_ironscales_ca_irontrapssettings_table')}}
    where
    date(_RIVERY_LAST_UPDATE) = current_date()
)
...
from
    ...
    left join setting on t.tenant_global_id = concat('CA-',setting.company_id)