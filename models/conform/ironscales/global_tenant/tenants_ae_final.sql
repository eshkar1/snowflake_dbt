...
with tenants_ae as (
    select * from {{ ref('tenants_ae_tree')}}
),
...
campaigns_company as (
    select * from {{ ref('stg_ironscales_ae_campaigns_company_table')}}
    where
    date(_RIVERY_LAST_UPDATE) = current_date()
...
auth_user as (
    select * from {{ ref('stg_ironscales_ae_auth_user_table')}}
    where
    date(_RIVERY_LAST_UPDATE) = current_date()
),

active_profiles as (
    select * from {{ ref('tenants_ae_active_profile')}}
),
...
select
    ...
    null::boolean as email_encryption_entitled,
    null::boolean as email_encryption_active,
    null::boolean as email_dlp_entitled,
    null::boolean as email_dlp_active,
    null::boolean as is_churned
