...
dmarc_hwm AS (
    SELECT * FROM {{ ref('feb_2026_dmarc_sltp_history') }}
),
...
-- ============================================================
-- PRICING RESOLVED: apply promo override logic
-- For each TENANT_GLOBAL_ID + SKU + IS_NFR:
--   - If an active Promo Price exists at end of Feb → use only promo rows
--   - Otherwise → use standard rows (anything that is not 'Promo Price')
-- Promo always wins. Falls back to standard when promo expires.
-- ============================================================
pricing_resolved AS (
    SELECT
        TENANT_GLOBAL_ID,
            ...
            MAX(
                CASE
                    WHEN p.PRICE_TYPE = 'Promo Price'
                     AND '2026-03-01'::date BETWEEN p.START_DATE AND p.END_DATE
                    THEN 1 ELSE 0
                END
    ...
    WHERE
        (has_active_promo = 1
            ...
            AND '2026-03-01'::date BETWEEN START_DATE AND END_DATE)
        OR
        (has_active_promo = 0
            AND PRICE_TYPE != 'Promo Price')
),
...
SELECT DATE_RECORDED, ltp, item, sku, partner_pricing, quantity, amount,
       CASE WHEN price_type = 'Promo Price' THEN 'Promo' ELSE 'Standard' END AS price_type
FROM plans
UNION ALL
SELECT DATE_RECORDED, ltp, item, sku, partner_pricing, quantity, amount,
       CASE WHEN price_type = 'Promo Price' THEN 'Promo' ELSE 'Standard' END AS price_type
