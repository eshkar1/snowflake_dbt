with sltp_pricing_function_all_ltps as (
    select * from {{ ref('SLTP_daily_pricing_function_for_all_ltps_no_hwm_updated')}}
),

sltp_pricing_function_disti as (
    select * from {{ ref('SLTP_daily_pricing_function_for_Disti_no_hwm_updated')}}
),

sltp_pricing_function_shareweb as (
    select * from {{ ref('SLTP_daily_pricing_function_for_shareweb_no_hwm_updated')}}
),

sltp_pricing_function_pax8_updated as (
    select * from {{ ref('SLTP_daily_pricing_function_for_pax8_no_hwm_updated')}} 
),
